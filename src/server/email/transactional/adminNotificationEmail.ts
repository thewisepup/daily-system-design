...
const ADMIN_NOTIFICATION_EMAIL = "wisepup257@gmail.com";

interface AutoNewsletterNotificationParams {
  topic: Topic;
  issue: Issue;
...
export async function sendAutoNewsletterNotification(
  ...
  const { topic, issue, wasAutoGenerated, wasAutoApproved, sequenceNumber } =
    params;
...
`;
  ...
  try {
    ...
    if (result.status === "sent") {
      console.log(
        `[AutoNewsletter] Admin notification sent to ${ADMIN_NOTIFICATION_EMAIL} (MessageId: ${result.messageId})`,
      );
    } else {
      console.error(
