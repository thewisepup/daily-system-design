...
/**
 * Obtain the current newsletter sequence, topic, and an approved issue for a subject.
 *
 * Ensures an approved issue exists for the topic, creating and/or auto-approving it if necessary.
 *
 * @returns An object containing:
 * - `issue`: the approved newsletter issue
 * - `sequence`: the newsletter sequence for the subject
 * - `topic`: the topic for the current sequence
 * - `wasAutoGenerated`: `true` if the issue was created automatically, `false` otherwise
 * - `wasAutoApproved`: `true` if the issue was auto-approved, `false` otherwise
 *
 * @throws TRPCError with code `NOT_FOUND` if the newsletter sequence cannot be retrieved/created or if the topic for the current sequence is missing.
 */
